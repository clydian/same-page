-- Additive: existing scores, PDF versions and old clients keep their contracts.
CREATE TABLE score_attachments (
  ...
  kind TEXT NOT NULL CHECK (kind IN ('link','audio','pdf','markdown')),
  name TEXT NOT NULL,
  url TEXT,
...
BEGIN INSERT OR IGNORE INTO score_object_deletions (id, object_key) VALUES (OLD.id, OLD.object_key); END;
...
-- No attachment revision history in v1. Unreferenced content is queued durably.
...
-- The platform ceiling includes physical retained data across both file families.
...
CREATE TRIGGER free_platform_storage_guard BEFORE INSERT ON score_versions WHEN
 ...
 (SELECT COALESCE(sum(f.size_bytes),0) FROM score_attachment_files f JOIN choirs c ON c.id = f.choir_id WHERE c.plan = 'free') + NEW.size_bytes >
