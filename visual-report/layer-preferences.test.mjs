...
for (const [engineName, engine] of Object.entries({ chromium, webkit })) {
  test(`${engineName}: reading preferences, colors and expanded layer panels remain usable`, async (t) => {
    ...
    const checkbox = page.getByRole("checkbox", { name: "Ensemble 默认显示" });
    ...
    await page.goto(`${origin}${drive}/preferences`);
    const display = page.getByRole("checkbox", { name: "Ensemble 默认显示" });
    ...
    await page.getByRole("button", { name: /^(编辑|完成编辑)$/, exact: true }).click();
    await page.getByRole("button", { name: /当前编辑层/ }).click();
    await page.getByRole("button", { name: "Soprano，只读，查看权限说明" }).click();
    ...
    await page.getByRole("link", { name: /Ensemble.*已授权/ }).waitFor();
    ...
    await page.getByRole("button", { name: "删除 Ensemble", exact: true }).click();
    await page.getByRole("dialog", { name: "删除共享层「Ensemble」？" }).waitFor();
    ...
    await page.getByRole("link", { name: /Ensemble.*已授权/ }).waitFor({ state: "hidden" });
    await page.locator(".view-selector-option").filter({ hasText: "已删除层" }).click();
