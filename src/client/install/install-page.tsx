...
export default function InstallPage() {
  ...
  useEffect(() => {
    ...
    async function prepare() {
      try {
        ...
        if (active && !controller.signal.aborted) setError(cause instanceof Error && cause.message === "expired" ? "链接已过期或失效，请回原云盘重新点击“添加到桌面”。" : "暂时没能接回云盘，请重试。也可以回原云盘继续看谱。");
      }
    }
    void prepare();
  ...
  return <main className="install-page page-shell">
    ...
    <div className="install-page-card">
      <p className="install-eyebrow">{external ? "第 1 步 · 换到浏览器" : "最后一步 · 添加到桌面"}</p>
