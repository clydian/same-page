...
export function ReaderGuide({ layout, onDismiss }: { layout: "page" | "continuous"; onDismiss(): void }) {
  const edges = layout === "page";
    ...
    <Modal className="reader-guide__surface">
      <Dialog aria-label="阅读器使用指引" className="reader-guide__content">
        ...
        <div className="reader-guide__zones" data-edges={edges || undefined}>
          {edges && <div><ArrowLeft size={26} aria-hidden="true" /><strong>轻点左侧</strong><span>整页时，上一页</span></div>}
          <div><Hand size={28} aria-hidden="true" /><strong>{edges ? "轻点中央" : "轻点谱面"}</strong><span>显示或收起工具栏</span></div>
          {edges && <div><ArrowRight size={26} aria-hidden="true" /><strong>轻点右侧</strong><span>整页时，下一页</span></div>}
        </div>
