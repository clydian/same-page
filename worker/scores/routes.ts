  ...
  const rows = await measureServerTiming(context, "d1", () =>
    context.env.DB.prepare(
      `SELECT choirs.id AS drive_id, choirs.name AS drive_name,
              ...
              scores.updated_at, (SELECT count(*) FROM score_attachments a WHERE a.score_id = scores.id AND a.trashed_at IS NULL AND a.purged_at IS NULL AND (${context.req.header("X-Same-Page-Attachments") === "2" ? "1" : "a.kind <> 'musicxml'"}) AND (a.kind = 'link' OR a.current_file_id IS NOT NULL)) AS attachment_count, versions.id AS version_id,
...
scoreRoutes.get("/choirs/:choirId/scores", async (context) => {
  ...
  const result = await measureServerTiming(context, "d1", () => context.env.DB.prepare(
    `SELECT scores.id, scores.choir_id, scores.file_name, scores.updated_at, (SELECT count(*) FROM score_attachments a WHERE a.score_id = scores.id AND a.trashed_at IS NULL AND a.purged_at IS NULL AND (${context.req.header("X-Same-Page-Attachments") === "2" ? "1" : "a.kind <> 'musicxml'"}) AND (a.kind = 'link' OR a.current_file_id IS NOT NULL)) AS attachment_count,
