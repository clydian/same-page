...
部署应让接受新可选字段的 Worker 与新前端一起发布，保留已有存储 schema。回滚时优先保留 Worker 的兼容解析；若缓存中的新客户端遇到旧 Worker 对扩展报告明确返回 400，会用相同报告 ID 去除这两个可选字段重试一次。网络失败或未知送达不触发格式降级，后续重试保留已发送的确切内容；没有重复创建反馈或业务请求重放。
