...
首次设置密码使用现有邮箱所有权验证能力：发送 `forget-password` 验证码，调用 `check-verification-otp` 验证后才显示密码表单；提交时由 `reset-password` 再次验证、原子消费验证码，并在原用户下增加密码认证身份。客户端的“已验证”状态不授予修改密码的权限。验证码仍以哈希存储，十分钟有效，允许三次错误尝试，邮件发送沿用邮箱冷却及邮箱/客户端限流；密码策略为 8–128 位。沿用密码重设邮件模板及会话撤销行为。
...
独立审查未发现实质规范违反或需报告的代码 smell。复用既有邮箱验证、密码策略、限流以及原用户的成员关系和个人层归属边界。审查指出的 README 分流描述已同步更新。

## Spec

独立审查未发现缺失、实质范围偏离或错误实现。审查同时核对了认证库的 OTP 预验证、再次验证和原子消费路径；本地模拟与真实登录验收边界已明确。

合计：Standards 0 项遗留问题，Spec 0 项遗留问题。
