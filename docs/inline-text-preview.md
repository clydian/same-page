...
追加修改验证：类型检查、Lint、生产构建通过；118 项 Node 单元与 814 项客户端测试通过；现有 11 项 Chromium／WebKit 移动阅读器回归通过。只在现有手势测试文件补充下滑退出、取消、短拖、双指、放大、编辑、连续滚动顶部的必要边界，更新旧提示／谱名断言。Standards 和 Spec 双轴审查均发现同一处越界 CSS 改动（邀请码及登录分隔线网格），已恢复；无其他明确未解决项。浏览器已实测下滑返回云盘及阅读指引自动收起，真实设备触摸体验仍需 iPad 验收。
