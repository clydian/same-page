...
文字输入工具栏修订：顶部仅显示当前层与独立“完成”；字号、对齐、方形色块位于可收起的底部样式栏，随可见视口停靠键盘上方，无软键盘时停靠屏幕底部。避让同时计入上下控件与移动手柄，平移整张谱面，保存中心锚点不变。移除取消和更多删除：点空白、Esc 或完成均提交；清空新文字不创建，清空已有文字后完成删除，保留撤销及已完成笔记拖动删除。
