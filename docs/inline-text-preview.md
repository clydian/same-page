...
连续滚动模式不再支持下拉退出，包括顶部反复下拉；仅翻页阅读的整页状态保留下滑关闭。滚动模式指引改为“上下滑动／连续浏览”，通过工具栏返回关闭乐谱。

连续滚动模式也取消横向滑动翻页（包括编辑时双指横滑），保留滚动、缩放与放大后的平移。指引只显示上下滑动浏览和双击缩放；左右滑动翻页仅用于翻页模式。

进一步清理：连续滚动彻底脱离横向翻页状态机、动画、目标页预渲染与翻页失败入口；缩略图跳转直接定位纵向列表。指引仅按翻页、连续滚动区分，不随缩放变化；在固定说明中解释整页与放大时的操作。帮助页按模式区分编辑时双指横滑是否换页。

文字输入工具栏修订：移除右上完成按钮，样式栏上方以“轻点空白处完成”轻提示说明结束方式；当前图层、字号、对齐、方形色块位于输入期间常显的底部样式栏，随可见视口停靠键盘上方，无软键盘时停靠屏幕底部。避让同时计入上下控件与移动手柄，平移整张谱面，保存中心锚点不变。移除取消和更多删除：点空白、Esc 或完成均提交；清空新文字不创建，清空已有文字后完成删除，保留撤销及已完成笔记拖动删除。

样式栏不再提供折叠状态或展开按钮；个人层图层名仅显示名称，文字颜色只由右侧方形颜色控件表示；共享层在图层名旁保留小方形只读色标，不提供颜色控件。每种情况下颜色仅出现一次。
